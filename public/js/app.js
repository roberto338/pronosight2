...
import { callClaude, callGemini, extractText, extractJSON, tsdbFetch, getLeagueEvents,
         tsdbToMatch, fdFetch, fdToMatch, fetchRealOdds, fetchApiStatus } from './modules/api.js';
...
async function analyze() {
  ...
  if (!isLive) {
    const cached = getCachedAnalysis(t1, t2, league);
    if (cached) {
      processAnalysisResult(cached, t1, t2, league, true);
      btn.disabled = false;
  ...
  const leg1Score = (document.getElementById('leg1Score') || { value: '' }).value.trim();
  const leg1Ctx = leg1Score ? ' | Score aller: ' + leg1Score : '';
  ...
  try {
    ...
    try {
      const searchData = await callGemini([{
        role: 'user',
        content: `${t1} vs ${t2} (${league} ${matchDate}): forme recente, blessures, h2h, cotes.`
      }], { useSearch: true, maxTokens: 1500 });
      webInfo = extractText(searchData);
    } catch { webInfo = 'Recherche web indisponible'; }

    const prompt = `Tu es un expert mondial en analyse sportive. Analyse ce match en FRANÇAIS uniquement.
Match : ${t1} vs ${t2} | Ligue : ${league} | Date : ${matchDate} | Live : ${isLive}

INFOS COLLECTÉES EN TEMPS RÉEL :
${webInfo.slice(0, 2000)}
...
Retourne UNIQUEMENT un JSON valide avec cette structure:
{
  ...
  "market_btts": "Oui",
  "market_btts_conf": 55,
  "market_over_line": "2.5",
  "market_over": "Over",
  "market_over_conf": 60,
  "market_handicap": "-1 Dom",
  "market_handicap_conf": 45,
  "best_bet": "Victoire domicile",
  "best_bet_market": "1",
  "best_bet_confidence": 70,
  "stars": 4,
  "traffic_light": "vert",
  "analysis": "Analyse experte en 3-4 phrases",
  "simple_explanation": "Explication simple pour un enfant",
  "team1_form": ["W","W","L","D","W"],
  "team2_form": ["L","D","W","L","W"],
  "blessures_team1": [],
  "blessures_team2": [],
  "compo_team1": "4-3-3: Gardien; Défense; Milieu; Attaque",
  "compo_team2": "4-4-2: Gardien; Défense; Milieu; Attaque",
  "key_factors": [{"icon": "🔥","text": "Facteur clé"}],
  "odds_home": 1.65,
  "odds_draw": 3.80,
  "odds_away": 5.20,
  "odds_source": "Bookmaker"
    ...
    const data = await callGemini([{ role: 'user', content: prompt }], { maxTokens: 3000 });
    let d = extractJSON(extractText(data));
    
    if (!d) throw new Error('JSON invalide');

    d.proba_home = d.proba_home || 40;
    d.proba_away = d.proba_away || 30;
    d.proba_draw = d.proba_draw || (100 - d.proba_home - d.proba_away);
    d.best_bet_confidence = d.best_bet_confidence || 60;
    d.stars = d.stars || Math.ceil(d.best_bet_confidence / 20);
    d.traffic_light = d.traffic_light || (d.best_bet_confidence >= 70 ? 'vert' : d.best_bet_confidence >= 50 ? 'orange' : 'rouge');
    d.team1_form = d.team1_form || ['?','?','?','?','?'];
    d.team2_form = d.team2_form || ['?','?','?','?','?'];
    d.key_factors = d.key_factors || [];
    d.blessures_team1 = d.blessures_team1 || [];
    d.blessures_team2 = d.blessures_team2 || [];

    setCachedAnalysis(t1, t2, league, d);

    let realOddsData = null;
    if (state.apiStatus?.odds && state.selectedLeague) {
      try {
        realOddsData = await fetchRealOdds(t1, t2, state.selectedLeague.id);
        if (realOddsData) {
          d.odds_home = realOddsData.best.home;
          d.odds_draw = realOddsData.best.draw;
          d.odds_away = realOddsData.best.away;
          d.odds_source = realOddsData.count + ' bookmakers';
        }
      } catch { /* continue */ }
    }

    processAnalysisResult(d, t1, t2, league, false, realOddsData);
  } catch (e) {
    console.error(e);
...
function renderResults(d, evData, kellyData, leg1Score, realOddsData) {
  ...
  const globalConf = computeGlobalConfidence(d, evData);
